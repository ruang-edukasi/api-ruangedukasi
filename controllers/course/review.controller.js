        ...
        try {
            ...
            if (data.length == 0) {
                return res.status(200).json({
                    error: false,
                    message: "Data ulasan dan penilaian tidak tersedia untuk kursus ini",
                });
            }

            return res.status(200).json({
                error: false,
                message: "Memuat review kursus berhasil",
                response: data,
            });
        } catch (error) {
    ...
    addCourseReview: async(req, res) => {
        try {
            ...
            if (checkAlreadyReview != 0) {
                return res.status(200).json({
                    error: false,
                    message: "Kursus ini sudah ditinjau dan diberi peringkat",
                });
            }
            ...
            return res.status(201).json({
                error: false,
                message: "Ulasan dan peringkat berhasil ditambahkan",
                response: data,
            });
        } catch (error) {
            console.error("Error:", error);
