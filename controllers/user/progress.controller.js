    ...
    try {
        ...
        if (checkValidContent == 0) {
            return res.status(404).json({
                error: true,
                message: `Kursus #${courseId} belum berisi #${contentId}`,
            });
        }
        ...
        if (checkValidUserContent == 0) {
            return res.status(404).json({
                error: true,
                message: `Anda belum memiliki kursus ini`,
            });
        }
        ...
        if (countProgress == 0) {
            try {
                ...
                return res.status(201).json({
                    error: false,
                    message: "Berhasil menambah kemajuan",
                    response: data,
                });
            } catch (error) {
                console.log(error);
        ...
        return res.status(200).json({
            error: false,
            message: "Kemajuan sudah ditambahkan",
        });
    } catch (error) {
        console.log(error);
        return res.status(500).json({
